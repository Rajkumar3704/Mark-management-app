class FacultiesController < ApplicationController
  ...
  def index
    ...
    if current_user.role == "faculty" 
      @faculties = Faculty.where(user_id: current_user.id)
    else
  ...
  # POST /faculties or /faculties.json     faculty_url(@faculty)
  def create
  ...
  # PATCH/PUT /faculties/1 or /faculties/1.json   ,faculty_url(@faculty)
  def update
    respond_to do |format|
  ...
  private
    ...
    def faculty_params
      params.require(:faculty).permit(:first_name, :last_name, :Faculty_id, :gender, :dob, :address, :email, :phone, :department, :user_id)
    end
end
